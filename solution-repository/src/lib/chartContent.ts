...
type StoredAsset = {
  file?: StoredMedia | string | number | null;
...
function mapCaseStudyAsset(asset?: StoredAsset | null): ChartCmsAsset | undefined {
  const uploadedFile = mapMedia(asset?.file);
  const externalFile = mapAsset(asset);
  const mappedAsset = uploadedFile ?? externalFile;
  ...
  return {
    ...mappedAsset,
    filename: asset?.title ?? mappedAsset.filename ?? asset?.filename ?? undefined,
  };
}

function mapMedia(
  media?: StoredMedia | string | number | null,
): ChartCmsAsset | undefined {
...
function toAssetRows(values?: ChartCmsAsset[]) {
  return (
    values
      ...
      .map((asset) => ({
        file: relationshipId(asset),
        title: asset.filename,
        filename: relationshipId(asset) ? undefined : asset.filename,
        url: relationshipId(asset) ? undefined : asset.url,
        type: relationshipId(asset) ? undefined : asset.type,
        size: relationshipId(asset) ? undefined : asset.size,
      })) ?? []
  );
}
...
export function mapDraftToContentData(draft: ChartCmsDraftInput) {
  ...
  return {
    ...
    body: draft.body,
    tag: solution.solutionTypes?.[0] ?? (draft.tag as SolutionTypeValue),
