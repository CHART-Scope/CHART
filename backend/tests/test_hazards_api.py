...
def test_bundled_solution_snapshot_is_available_without_the_legacy_api() -> None:
    with patch("chart.solution_repository.routes._remote", return_value=None):
        response = TestClient(app).get("/solutions?limit=1")

    assert response.status_code == 200
    assert response.json()["total"] > 0
    assert response.json()["items"][0]["id"].startswith("solution-")
