...
| Part | Owner | Rule |
|---|---|---|
| Web | `new_design/` | current planning UI; display and browser/session forwarding only |
| Older web shell | `web/` | retained during UI migration; not the current planning surface |
...
deployment, and migration path have been removed. The remaining JSON file under
`api/` is local solution-repository data, not an API service.
...
The connected planning page in `new_design` shows:
...
5. starts the Python API, Dagster, `new_design`, Keycloak, and optional R scorer;
...
- production deployment of `new_design` rather than the retired web shell;
...
- current end-to-end proof: request `21` used saved C3S August–October values
  ...
  `1.0562–1.3690`;
- connected `new_design` proof: authenticated request `24` displayed those
