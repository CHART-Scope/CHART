#!/usr/bin/env Rscript
# Scoring contract for sanitized CHART temperature-DLNM bundles.
...
validate_compact_bundle <- function(bundle) {
  ...
  supported_families <- c(
    "lbw_temperature_dlnm",
    "mortality_temperature_case_crossover_dlnm"
  )
  if (!(bundle$model_family %in% supported_families)) {
  ...
  if (identical(bundle$model_family, "mortality_temperature_case_crossover_dlnm")) {
    if (is.null(bundle$output$effect_measure) ||
        !identical(bundle$output$effect_measure, "odds_ratio")) {
      stop("Mortality compact model must declare output.effect_measure = odds_ratio")
    }
  }
  ...
  for (area in names(bundle$areas)) {
    if (identical(bundle$model_family, "lbw_temperature_dlnm")) {
      ...
      windows <- names(blocks)
      if (!is.list(blocks) || !length(blocks) || is.null(windows) ||
          anyDuplicated(windows) || !all(windows %in% as.character(1:3))) {
        stop("Area ", area, " must contain a named subset of pregnancy windows 1, 2, and 3")
      }
      for (window in names(blocks)) {
        validate_compact_block(blocks[[window]], area, window, "n_lbw_events")
      }
    } else {
      validate_compact_block(bundle$areas[[area]], area, "association", "n_events")
      n_model_rows <- as.integer(
        if (is.null(bundle$areas[[area]]$n_model_rows)) {
          bundle$areas[[area]]$n_training
        } else {
          bundle$areas[[area]]$n_model_rows
        }
      )
      if (length(n_model_rows) != 1 || is.na(n_model_rows) ||
          n_model_rows != as.integer(bundle$areas[[area]]$n_training)) {
        stop("Compact mortality model n_model_rows must equal n_training")
      }
      n_subjects <- as.integer(bundle$areas[[area]]$n_subjects)
      if (length(n_subjects) != 1 || is.na(n_subjects) || n_subjects < 1) {
        stop("Compact model block ", area, "/association has invalid subjects")
      }
    }
  }
  invisible(bundle)
...
validate_compact_block <- function(
  block,
  area = "",
  window = "",
  event_field = "n_lbw_events"
) {
  required <- c(
    "basis", "coefficients", "vcov", "reference_temperature_c",
    "modelled_temperature_range_c", "n_training", event_field
  )
  ...
  n_events <- as.integer(block[[event_field]])
...
compact_area_block <- function(store, area, pregnancy_window) {
  ...
  if (is.null(block)) {
    stop(
      "MODEL_PREGNANCY_WINDOW_NOT_AVAILABLE: area ", matched,
      " has no modeller-supplied window ", pregnancy_window
    )
  }
  list(
    area = matched,
...
score_compact_profile <- function(block, tmax_lag, ref = NULL) {
  ...
  expected_values <- as.integer(diff(as.numeric(block$basis$lag)) + 1L)
...
compact_association_block <- function(store, area) {
  if (!identical(
    store$bundle$model_family,
    "mortality_temperature_case_crossover_dlnm"
  )) {
    stop("Compact bundle is not a mortality association model")
  }
  if (is.null(area) || length(area) != 1 || !nzchar(trimws(area))) {
    stop("area is required")
  }
  ...
  list(area = matched, block = store$bundle$areas[[matched]])
}

score_compact_association <- function(store, area, exposure_values_c, ref = NULL) {
  selected <- compact_association_block(store, area)
  block <- selected$block
  result <- score_dlnm_parameters(
    basis = block$basis,
    coefficients = block$coefficients,
    covariance = block$vcov,
    tmax_lag = exposure_values_c,
    ref_temp = if (is.null(ref)) block$reference_temperature_c else ref,
    support = block$modelled_temperature_range_c,
    n_training = block$n_training,
    n_lbw_events = block$n_events
  )
  result$n_events <- result$n_lbw_events
  result$n_lbw_events <- NULL
  result$n_model_rows <- as.integer(
    if (is.null(block$n_model_rows)) block$n_training else block$n_model_rows
  )
  result$n_subjects <- as.integer(block$n_subjects)
  result$effect_measure <- store$bundle$output$effect_measure
  result$exposure_values_c <- result$tmax_lag
